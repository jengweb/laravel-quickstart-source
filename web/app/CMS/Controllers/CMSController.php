...
class CMSController extends BaseController
{
    ...
    {
        if (env('APP_DEBUG')) {
            $this->middleware('doNotCacheResponse', ['only' => ['index', 'preview', 'getSiteMapXML']]);
        } else {
            $this->middleware('doNotCacheResponse', ['only' => 'preview', 'getSiteMapXML']);
        }
    }
    ...
    public function index(Request $request)
    {
        ...
        $splitUrl = explode('/',trim($fullURl,'/'));

        if ( ! CMSHelper::checkIfTemplateExists('page')) {
            return abort(BaseResponse::HTTP_NOT_FOUND);
        }

        if ($pageData = APIHelper::getPageData(count($splitUrl)>=2 ? $splitUrl[count($splitUrl)-1] : null)) {
...
//            $urls = Cache::get(CMSConstants::URL_LIST, []);
//            $current = url()->current();
//            if ( ! in_array($current, $urls)) {
//                $urls[] = $current;
//                /** @noinspection PhpUndefinedMethodInspection */
//                Cache::forever(CMSConstants::URL_LIST, $urls);
//            }
     ...
     * @return mixed
     */
    public function getSiteMapXML(/** @noinspection PhpUnusedParameterInspection */ Request $request)
    {
    ...
    public function renderPaginator(Request $request)
    {
        ...
        $paginator = json_decode(json_encode($paginator), false);

        $controlListPaginator = new ControlListPaginator($paginator);

        $renderedTemplate = $controlListPaginator->render()->toHtml();
