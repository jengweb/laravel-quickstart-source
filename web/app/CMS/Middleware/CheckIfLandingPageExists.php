...
class CheckIfLandingPageExists
{
    ...
    {
        if (config('cms.landing.mode') == false || strtolower(config('cms.landing.mode')) == 'false') {
            return $next($request);
        }
    ...
    {
        $landingPage = public_path() . '/' . config('cms.landing.landing_folder') . '/' . config('cms.landing.landing_page');
